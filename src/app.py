...
def image_api():
    ...
    with open('/Users/leegangju/Documents/GitHub/CleanData/src/running.txt', 'r') as f:
        running = f.readline().strip()
    f.close()

    if running == 'True':
        # return {"message": "Face Detection is in Progress"}
        return requests.post('http://localhost:8080/image/receive', "Face Detection is in Progress")
    ...
    send_csvfile()
    return requests.post('http://localhost:8080/image/receive', "File(s) successfully uploaded")
def send_csvfile():
    ...
    f_date.close()
    print('8.Depplearning Start')
    # 딥러닝 모델 작동
    run(earliest_date)
    print('9.Deeplearning Finished')
